...
def load_data(args, small_sample=None):
    ...
    # MY CHANGES: Use the updated keys 'train', 'valid', and 'test'
    dataloader = {
        'train': train_loader['train'],  # changed from 'train_2'
        'valid': train_loader['valid'],  # changed from 'valid_2'
        'test': test_loader['test']  # changed from 'test_3'
    }
    return dataloader
...
def small_sample():
    ...
    for n in [1, 2, 3, 4]:
        for i in range(6):
            ...
            for e in range(10):
                save_folder = f'results/XJTU results (small sample {n})/' + str(i) + '-' + str(i) + '/Experiment' + str(
                    e + 1)
                if not os.path.exists(save_folder):
                    os.makedirs(save_folder)
                log_dir = 'logging.txt'
